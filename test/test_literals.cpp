...
TEST(LiteralsTest, getDouble) {
  ...
  ASSERT_EQ(Literals(Literals::LiteralsType::PI, 3, 1).getDouble(),
            3 * picolator::math::PI::value);
  ASSERT_EQ(Literals(Literals::LiteralsType::PI, 1, 3).getDouble(),
            pow(picolator::math::PI::value, 3));
  ASSERT_EQ(Literals(Literals::LiteralsType::PI, 3, 4).getDouble(),
...
TEST(LiteralsTest, Addition) {
  ...
  // Const
  ASSERT_EQ(Literals(Literals::LiteralsType::PI, 13, 1) +
                Literals(Literals::LiteralsType::PI, 4, 1),
            Literals(Literals::LiteralsType::PI, 17, 1));

  // Const with power
  ASSERT_EQ(
      (Literals(Literals::LiteralsType::PI, 1, 3) +
       Literals(Literals::LiteralsType::PI, 1, 5))
          .getDouble(),
